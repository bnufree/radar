...
void ZCHXLowranceRadarDataServer::analysisRadar(const QByteArray &sRadarData, const QString &sRadarType, int uLineNum, int uCellNum, int uHeading)
{
    ...
    if(prt)
    {
        QDir dir;
        dir.cd("../");  //进入某文件夹
        if(!dir.exists("VideoData"))//判断需要创建的文件夹是否存在
        {
            dir.mkdir("VideoData"); //创建文件夹
        }
        QString file_name ="../VideoData/VideoData_" + QString::number(name_num) + ".dat";
...
//        if(a > 30720) //当文本大于30M时 新建另一个文本写入数据       
        ...
        {
            name_num++;
            mVideoFileList.append(file_name);
            if(mVideoFileList.size() >= 10)
            {
                zchxRadarVideoWinRarThread* thread = new zchxRadarVideoWinRarThread(mVideoFileList);

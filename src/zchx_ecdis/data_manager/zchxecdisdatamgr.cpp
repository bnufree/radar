...
QList<QAction*> zchxEcdisDataMgr::getRightMenuActions(const QPoint &pt)
{
    ...
    if(mDisplayWidget)
    {
        Element* ele = mDisplayWidget->getCurrentSelectedElement();
        if(ele)
        {
            QAction* action = new QAction(tr("删除"),this);
            connect(action, &QAction::triggered, [=]()
            {
                QMessageBox::StandardButtons btns = QMessageBox::Ok | QMessageBox::Cancel;
                QMessageBox box(QMessageBox::Information, QString::fromUtf8("提示"), tr("删除选中的图元？"), btns);
                box.setButtonText(QMessageBox::Ok, QString::fromUtf8("确认"));
                box.setButtonText(QMessageBox::Cancel, QString::fromUtf8("取消"));
                if(box.exec() == QMessageBox::Ok)
                {
                    emit mDisplayWidget->signalRemoveItem(ele->getElementType(), ele->getID().toInt());

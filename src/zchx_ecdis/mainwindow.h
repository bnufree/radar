...
signals: //定义发送信号接口
    ...
    void itfNewRadarFilterArea(std::vector<std::pair<double, double>>);  // 选择回放区域
    void itfEditRadarFilterArea(std::vector<std::pair<double, double>>);  // 选择回放区域
    ...
    void signalSendCurPosAsRadarCenter(double, double);
    void signalRemoveItem(int type, int id);
public slots: //定义Recive数据接口
    ...
    void itfSetRadarVideoData(const QList<ZCHX::Data::ITF_RadarVideoImage>& list);
